 ...
 * dingtao:copy the code of DialChart.Modify to support Clock
 */
public class ClockChart extends RoundChart {
   ...
   */
  public ClockChart(CategorySeries dataset, DialRenderer renderer) {
  ...
  private double getAngleForValue(double value, double minAngle, double maxAngle, double min,
    ...
    double diff = max - min;
    //dingtao add 180 degree
    return Math.toRadians(180+minAngle + (value - min) * angleDiff / diff);
  }
    ...
    for (double i = min; i <= max; i += ticks) {
      ...
      if (labels) {
        ...
        if (Math.round(i) == (long) i) {
          text = (long) i + "";
          //将钟表上的3 6 9 12改为汉字 右 后 左 前---macaixia--2014.5.5
          if(i == 3){text = "右";}
          else if (i==6) {text= "后";}
          else if (i==9) {text= "左";}
          else if (i==12) {text= "前";}
        ...
        //dingtao avoid texts ovrides,don't show the first value
        if(((maxAngle % 360) ==minAngle || (minAngle % 360) ==maxAngle) && i == min){
        	continue;
        }
   ...
   *           series renderers
   */
  public static final GraphicalView getClockChartView(Context context, CategorySeries dataset,
	      DialRenderer renderer) {
	    checkParameters(dataset, renderer);
	    ClockChart chart = new ClockChart(dataset, renderer);
